...
import {
  SearchStackScreens,
  type SearchNavRouteProp,
  type SearchNavNavigationProp,
} from "@constant/NavigationTypes";

function SearchDetailScreen(): React.JSX.Element {
  ...
  const route = useRoute<SearchNavRouteProp<SearchStackScreens.SearchDetail>>();
  const navigation =
    useNavigation<SearchNavNavigationProp<SearchStackScreens.SearchDetail>>();
  ...
  const handlePress = (weather: WeatherProps) => {
    navigation.navigate(SearchStackScreens.SearchDayDetail, {
      city,
      weather,
    });
  };
